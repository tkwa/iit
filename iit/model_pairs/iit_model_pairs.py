...
class IITModelPair(BaseModelPair):
    ...
    def do_intervention(self, base_input, ablation_input, hl_node:HookName, verbose=False):
        ...
        assert all(hl_ablation_output == ablation_y),\
    ...
    def run_eval_step(self, base_input, ablation_input, hl_node:HookName, 
    ...
    def get_IIT_loss_over_batch(self, base_input, ablation_input, 
                                hl_node:HookName, loss_fn:Callable[[Tensor, Tensor], Tensor]):
        hl_output, ll_output = self.do_intervention(base_input, ablation_input, hl_node)
        loss = loss_fn(ll_output, hl_output)
        return loss
    
    def run_train_step(self, base_input, ablation_input, 
                       hl_node:HookName, loss_fn:Callable[[Tensor, Tensor], Tensor], 
                       optimizer:t.optim.Optimizer):
        optimizer.zero_grad()
        loss = self.get_IIT_loss_over_batch(base_input, ablation_input, hl_node, loss_fn)
        loss.backward()
        optimizer.step()
        return loss.item()
    ...
    def _run_train_epoch(self, loader, loss_fn, optimizer):
        ...
        for i, (base_input, ablation_input) in tqdm(enumerate(loader), total=len(loader)):
            ...
            losses.append(self.run_train_step(base_input, ablation_input, hl_node, loss_fn, optimizer))
        return losses
    ...
    def train(self, base_data, ablation_data, 
              test_base_data, test_ablation_data, 
              epochs=1000, use_wandb=False):
        training_args = self.training_args
        print(f"{training_args=}")
        dataset = IITDataset(base_data, ablation_data)
        test_dataset = IITDataset(test_base_data, test_ablation_data)
        loader = DataLoader(dataset, batch_size=training_args['batch_size'],
